...
//
// ── 왜 거리를 세 개나 재는가 ────────────────────────────────────────────────
// 명령→모터(speed_to_erpm_gain)와 모터→odom(erpm_to_speed)은 **별개 게인의 별개 경로**라
// "명령 1m vs 자 0.9m" 하나만 보면 어느 쪽이 틀렸는지 못 가른다. 그래서 한 번의 주행에서
// 독립적인 거리 3개를 동시에 적분한다:
//   1) 명령 = ∫ /drive.speed dt        2) 휠 = ∫ |odom twist.x| dt        3) 맵 = |끝−시작|
// `/pf/pose/odom` 한 토픽에 twist(VESC 패스스루)와 pose(MCL 추정)라는 **출처가 다른** 두
// 신호가 들어있는 게 핵심 — 2)와 3)이 어긋나면 자를 대기 전에도 스케일 오차가 드러난다.
//   휠≠맵 → odom 스케일(erpm_to_speed/휠반지름) / 명령≠휠 → speed_to_erpm_gain 또는 속도추종
//   맵≠자 → 맵 resolution 또는 MCL
//
// ⚠️ 3)은 **직선변위**다(경로길이 Σ|Δpos| 아님). pose는 "스캔매칭 앵커 + 그 이후 휠 변위"라
//    앵커에서 뒤로 당겨지는 보정 점프가 절댓값 합에 그대로 더해져 경로길이는 항상 과대평가된다
//    (지터 ±5cm에서 경로길이 +14% vs 직선변위 +0.45%). 경로길이는 "직진도" 판정에만 쓴다.
//
// 사용법: `ros2 launch f1tenth_control dashboard.launch.py mode:=calib` (무선이면
//   ROS_DISCOVERY_SERVER 필요). 주행은 수동 직선만 — 출발/정지를 자동 감지해 구간을 끊는다.
//   `/odom_calib/reset`(std_msgs/Empty)으로 이력 초기화.
//
// ⚠️ 측정 품질 직결:
//   - **직선만.** 화면의 "직진도"(직선변위/경로길이)가 0.99 미만이면 그 주행은 버릴 것.
//   - **5~10m로.** 1m는 자 대는 오차 ±2cm가 그대로 2% 불확실성이 된다(10m면 0.2%).
//   - **1.0~2.0 m/s.** VESC FOC 센서리스 데드존(800~2250 ERPM ≈ 0.17~0.49 m/s)을 피한다.
//   - **양방향 + 여러 속도.** 정/역 비대칭 = 오프셋 문제, 속도 의존 = 슬립 항(게인으로 못 고침).
//   - dt는 헤더 스탬프가 아니라 **우리 컴 수신시각**으로 적분한다(젯슨 시계 동기 보장 없음).
// ============================================================================
