...
- **`/scan` 불필요**(갭팔로워/AEB 없음 — AEB는 독립 노드, 비상 브레이크는 Mux 대행). LUT 불필요(전방 Pacejka 자체 모델). ⚠️ Pacejka는 gym 기본값 — 실차 보정은 별도 작업.
...
- 구독: `/joy`(원본: 조이스틱 드라이버 `joy_node`, `joy` 패키지 — `control_real.launch.py`가 `joy.launch.py`를 include해 함께 기동. 단독 실행: `ros2 launch f1tenth_control joy.launch.py`), `/drive_autonomous`(MAP), `/drive_mppi`(MPPI), `/aeb_active`
