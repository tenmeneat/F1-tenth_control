...
현재 `control_MAP.cpp`/launch 파일들에서 실제로 반복 확인된 패턴(`waypoint_topic`,
...
### 1. `control_map_node` (control_code/control_MAP.cpp) — 메인 자율주행 제어
...
- 구독: `/joy`(원본: 조이스틱 드라이버 `joy_node`, `joy` 패키지 — `control_real.launch.py`가 `joy.launch.py`를 include해 함께 기동. 단독 실행: `ros2 launch f1tenth_control joy.launch.py`), `/drive_autonomous`, `/aeb_active`
...
- 버튼 매핑: **LB(4)** AUTO/MANUAL 토글, **B(1)** 비상정지 Latch, **X(2)** 비상정지 해제, **A(0)** 부스트, **RB(5)** MAP/MPPI 알고리즘 전환(현재는 상태·대시보드 표시만, MPPI 노드가 없어 실제 `/drive_autonomous` 소스 라우팅은 미배선)
...
- 발행: `/aeb_active`(`std_msgs/Bool`)만 — 위험 감지 시 플래그만 올리고, 실제 brake는 Mux(joy_teleop_monitor)가 `/aeb_active`를 보고 `/drive`에 대행 송출(퍼블리셔 일원화)
- 3-point median 필터 → 전방 FOV 내 TTC 계산 → 임계치 미만/초근접 시 제동
- `latch_aeb=true`면 한 번 트리거 후 `/aeb/reset` 전까지 잠금. false면 위험 해제 1초 후 자율 복구.

### 4. `teleop_dashboard_node` (control_code/teleop_dashboard_node.cpp) — 대시보드 뷰어
...
### 5. `lut_calibrator_node` (control_code/lut_calibrator_node.cpp) — LUT 실측 보정 (관찰 전용)
...
### 6. `sim_imu_bridge_node` (control_code/sim_imu_bridge_node.cpp) — 시뮬 전용 odom→IMU 중계
...
플래닝팀 → /global_waypoints (WpntArray)
   ...
   /joy ──→ joy_teleop_monitor (Mux) ────────────/drive──→ 차량(VESC)
                    ↑
            /aeb_active ←── aeb_node (플래그만 발행, 제동은 Mux가 대행)
```
...
`declare_common_args()`에 추가해 여기로 이동 — 이제 `control_MAP.cpp`를 안 건드리고도 전부
터미널에서 튜닝 가능)
...
`base_max_decel`(8.0), `wall_safety_margin`(0.6), `use_imu`(true), `curvature_ff_blend`(0.0),
...
(sim=4.0, real=6.5). 바꾸려면 해당 launch 파일을 직접 수정.

### 향후: MPPI 노드 파라미터도 동일 패턴으로
`control_MPPI.cpp`에 `ControlMppiNode`가 붙으면(현재 컴파일 검증만 된 상태, 참고/비활성 자산
참조) 그 파라미터들도 위와 같은 방식으로 `_control_common.py`에 `declare_common_args()` +
`build_control_mppi_node()` 형태로 추가할 것 — `control_map_node`처럼 launch 파일이 아닌
코드(`control_MPPI.cpp`)는 안 건드리고 파라미터를 노출하는 패턴을 유지.
...
- `control_code/control_MPPI.cpp` (구 `mpc_controller.cpp`) — **샘플링 기반 MPPI 컨트롤러**(`MPPIController`). 정보이론 MPPI(Williams 2018): K개 잡음 롤아웃을 **동역학 자전거+Pacejka** 타이어 모델로 전진시켜 비용 가중평균으로 **조향+종가속 동시** 최적화. 저속(vx→0) 슬립각 발산은 기구학 자전거로 블렌드. 별도 헤더 없이 **단일 파일에 인라인 정의**(control_MAP.cpp 패턴)이며 외부 솔버 의존 없음(OSQP 제거). 현재 `CMakeLists.txt`에 정적 라이브러리(`control_mppi_solver`)로 **컴파일 검증만** 됨 — 아직 노드/`main()`/구독·발행 없음. 파일 하단 `#ifdef MPPI_SMOKE_TEST` 블록으로 ROS 없이 폐루프 검증 가능(`g++ -DMPPI_SMOKE_TEST`). ⚠️ 기존 NUC6 Pacejka LUT는 (횡가속도,속도)→조향각의 *역방향* 맵이라 MPPI 전방 롤아웃엔 못 씀 → 전방 Pacejka 파라미터는 자체 기본값(f1tenth_gym), 추후 실차 보정. **다음 세션**: 이 파일에 `ControlMppiNode`+`main()` 추가(같은 단일 파일), CMake `add_executable(control_mppi_node …)` 전환, `joy_teleop_monitor`의 RB 전환 상태(`current_algorithm_`)를 실제 소스 라우팅에 배선.
...
- 시뮬/실차 런치파일 공통 로직은 `launch/_control_common.py`에 있음 — 공통 파라미터 추가/변경
  시 여기 한 곳만 고치면 됨. 단 AEB·조이스틱 드라이버·`sim_imu_bridge_node` 포함 여부 같은
