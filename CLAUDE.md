...
- 차량: 휠베이스 0.33 m. **실제 최대 조향각 = 좌우 모두 약 23.5°(0.410 rad)** — 2026-07-31
  ...
  - 🔴 **현재 값: 좌 0.395 / 우 0.410** (2026-08-06). 우편향 보정으로 젯슨
    `steering_angle_to_servo_offset`을 0.4672 → **0.4591**로 0.8° 좌 트림했고, 중립이 좌로
    간 만큼 좌측 가동범위가 줄어 `(0.4591−0.23)/0.5785 = 0.3959`가 되어 좌만 낮췄다.
    **이 셋(offset·gain_left/right·max_steering_left/right)은 한 묶음이다** — 하나만 바꾸면
    풀락에서 servo가 `servo_min`(0.23)/`servo_max`(0.66)에 걸려 클리핑되고, 그 로그가 VESC
    시리얼을 굶겨 odom이 조용히 망가진다(아래 "작업 시 주의사항" 참고).
...
| `max_steering_left` / `max_steering_right` | **0.395 / 0.410** (real), 0.41 / 0.41 (sim) | 좌/우 조향 한계 [rad] = **실제 바퀴 각**(젯슨 좌우 개별 게인이 변환). 2026-08-06 우편향 0.8° 좌 트림(offset 0.4672→0.4591)에 맞춰 좌만 0.410→0.395. **젯슨 `vesc.yaml`의 `offset`(0.4591)·`gain_left`(−0.5785)/`gain_right`(−0.4702)·`servo_min`(0.23)/`servo_max`(0.66)과 반드시 한 묶음.** 조향 권한 캡은 둘 중 **작은 쪽**(이제 좌 0.395)을 씀 |
...
- 조향 한계(실차 좌 0.395 / 우 0.410 = 실제 바퀴 각, 시뮬 ±0.41), brake accel -9.0 등
