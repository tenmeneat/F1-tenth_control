...
### 6. 좌우 조향 비대칭 — 우조향이 8% 손해 (실측 도구 신설)

젯슨 `vesc.yaml`: gain −0.4463, offset 0.4672, `servo_min` 0.2703 / `servo_max` 0.6363.
`servo = gain·δ + offset`이므로

| 방향 | 한계 servo | 실제 δ |
|---|---|---|
| 좌 (δ>0) | 0.2703 | **+0.441 rad** (여유) |
| 우 (δ<0) | 0.6363 | **−0.379 rad** (0.41 대비 −7.6%) |

±0.41 대칭에 필요한 범위는 **[0.2842, 0.6502]**. `run_0727_203040`에서 실제로 servo 0.6502가
발행돼 `vesc_driver`의 `servo_limit_.clip()`에 잘렸다.
⚠️ `MAX_STEERING_ANGLE`(0.41)과 `steer_authority_ratio` 곡률 캡이 **좌우 대칭을 가정**하므로
**우선회 코너에서 캡이 낙관적**이다(실제보다 8% 더 꺾인다고 보고 속도를 덜 줄임).

`servo_max`가 소프트웨어 보수값인지 기구 한계인지 모르면 못 고친다 →
**`tools/servo_range_probe.py`** 신설: 바퀴를 띄운 채 0.005씩(≈0.011 rad) 올리며 매 스텝
확인받는 안전 탐색. 결과에 따라 `servo_max`를 0.6502로 올리거나(대칭 회복),
기구 한계면 `MAX_STEERING_ANGLE`을 낮춘다. **실차 벤치 필요 — 미측정.**
