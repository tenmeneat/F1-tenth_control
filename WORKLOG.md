...
- **`~/.zshrc`의 `ROS_DOMAIN_ID`가 `67`로 되돌아가 있음** — 07-17에 팀원 시뮬 충돌 때문에
  `67 → 88`로 바꿨던 수정이 사라짐(아래 07-17 항목 참고).
...
### 작업 2: `ROS_DOMAIN_ID` 재발 방지 (07-17 재발)
- [ ] 젯슨 `~/.zshrc`의 `ROS_DOMAIN_ID=67`이 왜 되돌아왔는지 확인(설정 유실/덮어쓰기 경위)
- [ ] 팀과 도메인ID 고정 배정 **조율 후** 확정값 적용. 67은 07-17에 충돌 이력이 있으므로 회피
- [ ] `~/.bashrc`에도 같은 값 추가 — 현재 zshrc에만 있어 **bash로 붙으면 도메인 0으로 떨어진다**
      (07-20 조사에서 실제로 이것 때문에 오판. SSH·스크립트·cron 등 비대화형 진입 경로 전부 영향)
- [ ] 시뮬 돌리는 팀원에게 `export ROS_LOCALHOST_ONLY=1` 공지 — 시뮬이 LAN으로 안 새게 하는
      가장 간단한 방법(도메인 조율 실패해도 안전)
- [ ] `/rviz` 이름 중복 2개 정리
