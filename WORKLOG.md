...
## 2026-07-14 — fuck_f1 프로덕션 트랙 랩타임 8.54→**6.97s**(7초 돌파): 속도 프로파일 재생성이 레버

### 배경/진단
"프로덕션 트랙(fuck_f1 + 우리 global_planner)에서 7초 만들기"가 목표. 먼저 데이터로 원인 규명:
우리 폐루프는 8.54s인데 프로파일 오프라인 추정이 8.27s → **컨트롤러는 이미 프로파일을 거의
따라잡고 있어 컨트롤러 튜닝으론 7초 불가**. 작년 new_map_con(7.1s)과 우리 프로덕션 프로파일을
직접 비교: new_map_con vx **min 5.0/mean 5.38**, 우리 **min 3.5/mean 4.43**. **즉 "뭐가 다른가 =
속도 프로파일이 보수적"**(우리 저속 지점들이 전부 생성기 min_speed=3.5 바닥에 클램프, generator
max_lateral_accel=5.8이 코너속도를 눌러서). 7초 필요 평균속도 5.10.

### 방법 — 프로파일 재생성 + 컨트롤러 MLA 동반 상향
`offline_trajectory_generator`를 헤드리스로 재생성(gui_params 로드 → make_namespace →
generate_trajectory → write_outputs, scratchpad `regen.py`). 생성기 속도 파라미터를 점진 상향하며
폐루프 실측. **핵심 커플링**: 프로파일 코너속도를 올리면 컨트롤러 backward-pass 그립클램프
(`max_lateral_accel`)가 프로파일 아래로 눌러버리므로 컨트롤러 MLA도 같이 올려야 온전히 추종.

### 결과 (시뮬 폐루프 실측)
| 프로파일 est | 컨트롤러 MLA | 랩타임 |
|---|---|---|
| 8.27(원본) | 6 | 8.54s |
| 7.56 | 8 | 7.77s |
| 7.028 | 10 | 7.16s |
| 6.892 | 10 | 7.02s |
| **6.847** | **10** | **6.97s (best 6.93)** ✅ |

**승리 생성기 파라미터**(gui_params 대비): `max_lateral_accel` 5.8→**9.8**, `max_accel` 5.8→**13.5**,
`min_speed` 3.5→**4.7**, `max_speed` 9.0→**11.5**. **승리 컨트롤러**: `max_lateral_accel=10`,
`base_max_accel=9`. 그 이상(프로파일 mean 5.4+, est<6.8) 밀면 컨트롤러 추종 한계로 2 m/s로
뭉개짐 — 6.85가 우리 L1+LUT 구조의 실질 바닥.

### 반영 & 안전 처리
- **컨트롤러**: `control_sim.launch.py` 기본값 `max_lateral_accel` 6.0→**10.0**, `base_max_accel`
  4.0→**9.0**(sim 전용, 공격적 프로파일 추종용). ⚠️ **시뮬 낙관치** — MLA=10은 LUT 실그립 피크
  (~6.7) 크게 초과라 실차 그대로 쓰면 코너 슬라이드. `control_real.launch.py`는 보수값 유지, 실차는
  실그립 매칭 프로파일 + MLA~6.7 필요. repo·2026_IFAC 반영 + 빌드.
- **프로파일**: 승리본은 scratchpad(`global_waypoints_WINNING_6.97s.json`)에 보관, **프로덕션 공유
  파일 `global_planner/data/global_waypoints.json`은 원본(8.274s)으로 복구**. 재생성은 글로벌
  파트원이 위 생성기 수치로 수행(공유 파일 임의 변경 방지). 실차 배포 전 실그립 검토 필수(헤어핀
  a_lat 14는 실타이어 초과).

### 겪은 함정 (재사용)
- **gym이 마지막 /drive 속도를 유지** → 컨트롤러 재시작 공백에 차가 조향 없이 직진해 이탈.
  프로파일/컨트롤러 바꿀 때마다 **깨끗한 gym 재스폰**(또는 0-속도 /drive 송출 후 교체) 필수.
  이걸 몰라 여러 설정을 "크래시"로 오판했다가 clean 재시작하니 전부 정상 추종(더 빠른 프로파일도).
- gym_bridge_launch가 ~50% 확률로 즉시 exit 1(원인불명) → 1회 재시도하면 뜸.
- MPPI 노드가 반복 launch 중 CUDA `bad_alloc`로 죽음(control_map_node엔 무영향, 무시 가능).

---

## 2026-07-13 (2) — 작년 실대회맵(oct_28) 벤치 → 곡률 사전감속을 "프로파일 신뢰형 backward-pass"로 교체

### 배경
작년 대회 참가 친구가 실차 랩타임 **7초 중반(~7.5s)**을 확인해줌. 사용자가 카카오톡으로 받은
작년 최종 맵·웨이포인트(`~/Documents/카카오톡 받은 파일/`: `oct_28.pgm/yaml`,
`oct28.csv`=148점 표준 f110 궤적)로 우리 `control_map_node`를 처음으로 그 실제 대회맵에서 폐루프
측정. `oct_28.pgm`은 `~/2026_IFAC/new_map_con/maps/`와 md5 동일(복사 불필요). 세팅: f1sim_C
gym_bridge를 oct_28+wp0 시작포즈로, `oct28.csv`를 스크립트로 `/global_waypoints`(WpntArray,
d_left/right=1.5·나머지 CSV값)로 latched 발행, ground-truth odom 사용(particle_filter 스킵).

### 진단 (텔레메트리 정량분석)
기본값 **13.62s**(7랩 일관, 스핀·이탈 0), 그러나 평균 실속도 2.24 m/s = 프로파일 평균(5.46)의
**41%**. 원인: 오프라인 프로파일 vx_mps가 **암묵 가정하는 횡가속도 `a_lat=vx²·κ`가 최대 10
m/s²(평균 6.62)**인데, 우리 곡률 사전감속이 고정 `max_lateral_accel=6.0`으로 `√(6/κ)` 캡을 걸어
**트랙의 66%에서 프로파일보다 낮게 눌렀고**, 게다가 캡이 **창 내 '최대 κ'**를 써서 다가오는 코너
하나가 그 앞 직선(프로파일 8 m/s)까지 3.5로 과잉감속시킴. 조향 포화는 0% → 조향이 아니라 순전히
속도캡 문제. 파라미터 스윕(구 코드): MLA 6→8→10 = 13.62→11.79→11.05s(수확체감), MLA=50(캡
사실상 제거)은 **이탈/크래시**(우리 L1+LUT 조향이 프로파일 진입속도에서 언더스티어 → 사전감속이
사실 그 약한 추종을 보상 중이었음).

### 변경 — 프로파일 신뢰형 backward-pass (control_map_node.cpp 1.5절)
기존 `curvature_speed_limit = √(max_lateral_accel/κ_max)` 블랭킷 재캡을 폐기하고 물리적으로 정확한
backward-pass로 교체:
- 전방 각 지점 i의 목표속도 `v_cap[i] = min(프로파일 vx[i], √(max_lateral_accel/κ_smoothed[i]))`
  (프로파일을 신뢰하되 실그립 초과 코너만 그립으로 한 번 더 누름)
- 그 목표까지 `base_max_decel`로 감속 가능한 현재 최대속도 `v_reach=√(v_cap[i]²+2·a·d_i)`의
  최소값을 캡으로 사용. accum=0(현재 위치) 항이 순간 그립 클램프를 자연히 겸함 → 별도 클램프 불필요.
- 직선·완만구간은 κ≈0 → 그립캡 무한대 → v_cap=프로파일(안 눌림), 코너는 제동거리만큼 앞에서부터
  정확히 그립속도로 선제동.
- **구현 중 2번 크래시로 교정**: ① 단순 min-over-window(코너속도 2m 일찍 부과, 여전히 과잉감속)
  ② 그립 클램프를 backward-pass 뒤에 따로(코너 정점서 뒤늦게 발동→뜨겁게 진입해 이탈). 최종본은
  그립을 각 지점에 녹여 둘 다 해결.

### 결과 (시뮬 실측)
- **oct_28**: 기본 MLA=6 → **12.07s(−11%)**. MLA 8/10 → 11.10/10.98s. **MLA 20~50에서도
  10.98s 안정** — 구 코드가 이탈하던 고 MLA에서 backward-pass는 항상 제대로 선제동해 **크래시
  실패모드 자체를 제거**(고 MLA에선 그립캡이 안 물려 순수 프로파일 추종으로 수렴, 그게 우리
  컨트롤러의 프로파일-추종 상한 ~11s). MLA는 이제 "실차 타이어 그립" 파라미터로 물리적 의미 명확.
  남은 11s→7.5s 갭은 사전감속이 아니라 다른 층(헤딩오차 감속·횡추종 능력) — 다음 과제.
- **fuck_f1 회귀**: 8.54s → **8.64s(+0.10s, +1.2%)**, 6랩 일관 완주. 미세 회귀지만 실제 대회맵
  (oct_28) 개선+크래시제거가 명백히 우선이라 **유지 결정**(사용자 확정).

### 검증
- `~/2026_IFAC`에서 `colcon build` 클린(⚠️ 빌드 중 발견: repo엔 있는 빈 `config/` 디렉토리가
  2026_IFAC 사본에 없어 install 실패 — git이 빈 디렉토리를 추적 안 해 동기화 누락. `mkdir config`로
  해결. CMakeLists `install(DIRECTORY launch config ...)`가 요구). repo·2026_IFAC 양쪽 동일 반영.
- oct_28/fuck_f1 폐루프 모두 스핀·이탈 없이 일관 완주 확인.
