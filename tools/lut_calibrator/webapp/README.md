...
- **베이스 LUT 선택**(2026-08-05 추가) — `표준`(조향축 상한 0.4000, 실효 천장 0.3897) ↔
  `축 확장`(상한 **0.4320**, 수동 풀락 실측용). 기본은 `표준`이라 **CLI 기본값과의 바이트
  동일성은 그대로**다. 전환 시 격자 크기가 달라지므로 누적 상태를 자동 초기화하고
  localStorage 키도 분리한다.
  🔴 축 확장본의 상단 4행은 **측정이 아니라 시드**다 — 자율 bag으로는 영원히 안 채워지고
  (조향 명령을 만드는 게 LUT 자신이라 0.3897에서 saturate = 닭-달걀), **수동 풀락 정상원
  주행**으로만 채워진다. 채우기 전 배포 금지. 상세는 [`../README.md`](../README.md) 참고
- **Prior weight 조절**(2026-08-05 추가) — CLI `--prior-weight`와 같은 뜻. 축 확장 워크플로에선
  시드가 아니라 실측이 지배해야 하므로 **0.5** 정도로 낮출 것
