...
public:
    ...
    double lookup_steer_angle(double accel, double vel) {
        ...
        // 각 속도 열은 조향각(row)이 커질수록 lat_acc가 단조 증가하다가 타이어가
        // 슬립각 한계(Pacejka 피크)를 넘으면 다시 감소하는 "봉우리형" 곡선이다(실측
        // 확인: 전 속도축에서 단일 피크). find_closest_neighbors는 배열에서 target에
        // 가장 가까운 값과 그 이웃으로 선형보간하는데, 피크를 넘는 목표 lat_acc(예:
        // 급커브에서 v^2 항 때문에 순간적으로 그리드 최대치를 초과)가 들어오면 피크
        // 양쪽(저조향/고조향)의 서로 다른 두 조향각이 "같은 정도로 가까운 값"이 되어
        // 매 사이클 어느 쪽이 선택되는지 진동 → 조향 채터링/포화가 반복되는 것으로
        // 판단됨(2026-07-13, 촘촘한 시케인 웨이포인트 스핀 원인 조사). 피크 이후
        // 구간을 NaN 처리해 검색을 "피크 이전(저조향, 그립 내)" 단조 구간으로 제한하면
        // 피크를 넘는 요청은 자연히 그 속도에서의 최대 그립 조향각으로 saturate되어
        // 항상 하나의 안정적인 해로 수렴한다(find_closest_neighbors는 첫 NaN에서 배열
        // 순회를 멈추므로 피크 이후를 NaN으로 채우기만 하면 됨, 알고리즘 변경 불필요).
        {
            size_t peak_idx = 0;
